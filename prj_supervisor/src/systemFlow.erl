...
-export([create/3,create/4,init/1,update/1,getSystemFlow/1,stopSystemFlow/1]).


create(ListOfPumpInstances,FlowMeter,Delay)->
    gen_server:start_link(?MODULE,[{ListOfPumpInstances,FlowMeter,Delay}],[]).

create(ListOfPumpInstances,FlowMeter,Delay,Number)->
    Atom = testFunctions:numberToAtom(systemFlow,Number),
    Exists = whereis(Atom),
    if(Exists==undefined)->
        gen_server:start_link({local,Atom},?MODULE,[{ListOfPumpInstances,FlowMeter,Delay}],[]);
    true->
        unregister(Atom),
        gen_server:start_link({local,Atom},?MODULE,[{ListOfPumpInstances,FlowMeter,Delay}],[])
    end.
