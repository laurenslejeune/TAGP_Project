...
-export([getSystemFlow/0,getSystemTemp/0,switchOnPumps/0,switchOffPumps/0,switchOffRandomPump/0,switchOnRandomPump/0]).
...
handle_cast(switch_on,{N_pipes,N_pumps,N_hex})->
    Pumps = pumpFlowmeterHESupervisor:generateNPumpIds(N_pumps+1,2*N_pumps),
...
handle_cast(crash,{N_pipes,N_pumps,N_hex})->
    PumpList = pumpFlowmeterHESupervisor:generateNPumpIds(1,2*N_pumps),
    HexList  = pumpFlowmeterHESupervisor:generateNHEXIds(1,2*N_hex),
    PipeList = fluidumSupervisor:generateNPipesIds(1,2*N_pipes),
