...
create({Begin_N,End_N},N_pumps,N_hex)->
    supervisor:start_link({local,?MODULE},?MODULE,[{Begin_N,End_N},N_pumps,N_hex]).

init([{Begin_N,End_N},N_pumps,N_hex])->
    ...
    SystemFlowSpec = #{id => systemflow_child,
                    start => {systemFlow, create, [,1]},
                    restart => permanent,
                    shutdown => brutal_kill,
                    type => worker,
                    modules => [heatExchangerTyp]},

    ChildSpecs = PumpSpecs++FlowMeterSpecs++HeatExchangerSpecs,
